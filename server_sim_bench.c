...
Compilation instructions:
    ...
    This program generate performance stats for the proxy server using different input sizes
...
void run_simulation(int MAXTHREADS, char* filename);
...
int main(int argc, char* argv[]) {
    ...
    double diff_t;
    // char* links[] = {"links/164.txt", "links/328.txt", "links/656.txt"};
    double data[3][7];
    char* links[argc-1];

    if(argc > 1) {
        for(int i = 1; i < argc; i++) {
            links[i-1] = strdup(argv[i]);
        }    
    } else {
        printf("Please input link files\n");
        exit(1);
    }
    ...
    // prints execution time of proxy server on varying amounts of requests and threads.
    for(int l = 0; l < argc-1 ; l++) {
        // printf("Problem Size: %s requests\n\n", links[l]);
        fprintf(fptr, "\n\n%s\n\n", links[l]); 
        for(int i = 0; i < 7; i++) {
            gettimeofday (&start, NULL);
            run_simulation(1 << i, links[l]);
            gettimeofday (&end, NULL);
            diff_t = (((end.tv_sec - start.tv_sec)*1000000L
                +end.tv_usec) - start.tv_usec) / (1000000.0);
            // printf("num_thr: %d\n", 1 << i);
            // printf("Reponse time: %f seconds", diff_t); 
            fprintf(fptr, "%f ", diff_t); 
            data[l][i] = diff_t;
            sleep(8);
        }
    }
...
void run_simulation(int MAXTHREADS, char* filename) {
    ...
    int req_cnt = load_requests(requests, filename);
...
void http_proxy(void* req) {
    ...
    request[num-1] = '\0'; 
    // force each thread to do both a read and a write to the cache
    // so load on cache is high
    ret = cache_get(request, num);
    ret = curl_http(request, num);
}
